// src/core_logic/door_operation_manager.cpp
...
#include "../io/feedback_manager.h" // Assurez-vous que FeedbackPatternType est accessible si besoin direct ici, sinon via les fonctions
...
const unsigned long AUTO_CLOSE_DELAY_MS = 15000;
const unsigned long ERROR_DISPLAY_HOLD_MS = 5000;
const unsigned long MOTOR_OPERATION_MAX_DURATION_MS = 20000;
...
static void log_and_display_state_change(DoorOpState previous_state, const char* event_trigger = "") {
    ...
    if (clear_lcd_line0) display_update_input_code("");
    if (!msg.isEmpty()) display_show_message(msg, (current_op_state == DoorOpState::LOCKED_OUT ? 0 : 1), true);
}

void door_op_manager_setup() {
    bool is_closed_now = door_sensor_is_door_fully_closed();
    bool is_open_now = door_sensor_is_door_fully_opened();
    DoorOpState old_state_for_log = current_op_state;
    if (is_closed_now && !is_open_now) current_op_state = DoorOpState::IDLE_CLOSED;
    else if (is_open_now && !is_closed_now) {
        current_op_state = DoorOpState::IDLE_OPEN;
        timer_activity_start_ms = millis(); auto_close_is_scheduled = true;
    } else current_op_state = DoorOpState::IDLE_AJAR;
    log_and_display_state_change(old_state_for_log, "SetupInitial");
    previous_door_closed_sensor_state_for_forcing_detection = is_closed_now;
...
void door_op_manager_task() {
    ...
    // ***** DEBUT DE LA CORRECTION POUR FORCAGE *****
    if (previous_door_closed_sensor_state_for_forcing_detection == true && current_physical_door_closed_state == false) {
        ...
        // CORRIGÉ: Suppression de "&& current_op_state != DoorOpState::LOCKED_OUT"
        // On veut détecter un forçage même si le système est en état LOCKED_OUT.
        if (!is_opening_legitimately) {
            Serial.println("DoorOpMgr_Task: !! ALERTE FORCAGE DE PORTE DETECTE !!");
            if (current_op_state == DoorOpState::LOCKED_OUT) { // Log supplémentaire si pendant lockout
                 Serial.println("  -> Occurred during LOCKED_OUT state!");
            }
            display_show_message("ALERTE FORCAGE!", 0, true); //
            feedback_signal_alert_forced_entry(); // Appel à la fonction qui demande le pattern non-bloquant
            ApiResponse alert_resp = api_send_alert("door_forced_open", "Detection d'ouverture de porte non commandee.", "critical"); //
            if (alert_resp.network_success && alert_resp.action_success) Serial.println("  -> Alerte de forçage envoyée.");
            else Serial.println("  -> Echec envoi alerte de forçage.");
            // Optionnel: Gérer une transition d'état spécifique pour forçage ici si nécessaire
        }
    }
    previous_door_closed_sensor_state_for_forcing_detection = current_physical_door_closed_state;
    // ***** FIN DE LA CORRECTION POUR FORCAGE *****
    ...
    if (input_manager_is_exit_button_newly_pressed()) { //
        ...
        if (current_op_state == DoorOpState::IDLE_CLOSED || current_op_state == DoorOpState::IDLE_AJAR ||
            ...
            if(current_op_state == DoorOpState::LOCKED_OUT) Serial.println("  -> Exit initiated while system was LOCKED_OUT.");
            if (agent_is_legitimately_inside) {
                ...
                if(exit_log_resp.network_success && exit_log_resp.http_code == 201 && exit_log_resp.action_success) Serial.println("  -> 'door_close' (_LBE_) notification sent.");
                else {Serial.print("  -> Failed 'door_close' (_LBE_) notification. HTTP: "); Serial.print(exit_log_resp.http_code); Serial.print(", R: "); Serial.println(exit_log_resp.reason);}
                agent_is_legitimately_inside = false;
            } else {
                ...
                if (alert_resp.network_success && alert_resp.action_success) Serial.println("  -> Suspicious exit alert sent.");
                else Serial.println("  -> Failed to send suspicious exit alert.");
                ApiResponse exit_log_resp = api_notify_door_close("");
                if(exit_log_resp.network_success && exit_log_resp.http_code == 201 && exit_log_resp.action_success) Serial.println("  -> 'door_close' (empty code for suspicious) notification sent.");
                else {Serial.print("  -> Failed 'door_close' (empty suspicious) notification. HTTP: "); Serial.print(exit_log_resp.http_code); Serial.print(", R: "); Serial.println(exit_log_resp.reason);}
            }
            if (door_sensor_is_door_fully_opened()) { //
                current_op_state = DoorOpState::IDLE_OPEN; auto_close_is_scheduled = true; timer_activity_start_ms = millis();
            } else {
                door_motor_open(); timer_activity_start_ms = millis(); //
                current_op_state = DoorOpState::MOTOR_OPENING; auto_close_is_scheduled = true;
            }
        } else {
    ...
    switch (current_op_state) {
        case DoorOpState::IDLE_CLOSED: case DoorOpState::IDLE_AJAR:
            if (!code_to_validate.isEmpty()) current_op_state = DoorOpState::PENDING_API_VALIDATION;
            break;
        case DoorOpState::IDLE_OPEN:
            if (auto_close_is_scheduled && (millis() - timer_activity_start_ms >= AUTO_CLOSE_DELAY_MS)) {
                ...
                if (!is_currently_closed_by_sensor) {
                    door_motor_close(); timer_activity_start_ms = millis(); current_op_state = DoorOpState::MOTOR_CLOSING; //
                } else {
                    auto_close_is_scheduled = false; current_op_state = DoorOpState::IDLE_CLOSED;
                    if (!last_valid_code_for_api_notify.isEmpty()) {
                        ApiResponse response = api_notify_door_close(last_valid_code_for_api_notify);
                        if(response.network_success && response.http_code == 201 && response.action_success) Serial.println("  -> API 'door_close' (finalize code for already_closed door): OK.");
                        else Serial.println("  -> API 'door_close' (finalize code for already_closed door): FAILED.");
        ...
        case DoorOpState::PENDING_API_VALIDATION:
            {
                if (!wifi_is_connected()) { //
                    feedback_signal_error(); current_op_state = DoorOpState::API_ERROR;
                    timer_activity_start_ms = millis(); code_to_validate = "";
                } else {
                    ApiResponse api_response = api_attempt_door_open(code_to_validate); //
                    if (api_response.network_success && api_response.http_code == 201 && api_response.action_success) {
                        ...
                        code_to_validate = ""; failed_code_attempts = 0;
                        if (!door_sensor_is_door_fully_opened()) {
                            door_motor_open(); timer_activity_start_ms = millis();
                            current_op_state = DoorOpState::MOTOR_OPENING; auto_close_is_scheduled = true;
                            display_show_message("Code Valide!", 0, false);
                        } else {
                            current_op_state = DoorOpState::IDLE_OPEN; timer_activity_start_ms = millis(); auto_close_is_scheduled = true;
                        }
                    } else {
                        Serial.print("DoorOpMgr_Task: API: Code INVALIDE or Error. Reason: "); Serial.println(api_response.reason);
                        display_show_message("Code Invalide", 0, false); display_show_message(api_response.reason.substring(0,15), 1, true);
                        if (!api_response.network_success || (api_response.http_code != 201 && api_response.http_code != 200 && api_response.http_code != 0)) {
                            ...
                            feedback_signal_error(); failed_code_attempts++;
                            ...
                            if (failed_code_attempts >= app_settings.max_failed_attempts) { //
                                ...
                                if (alert_api_resp.network_success && alert_api_resp.action_success) Serial.println("DoorOpMgr_Task: Alert sent successfully.");
                                else Serial.println("DoorOpMgr_Task: Failed to send alert.");
                        ...
                        if (current_op_state != DoorOpState::LOCKED_OUT) {
                            code_to_validate = "";
                            bool is_closed_after_fail = door_sensor_is_door_fully_closed(); bool is_open_after_fail = door_sensor_is_door_fully_opened();
                            if (is_closed_after_fail && !is_open_after_fail) current_op_state = DoorOpState::IDLE_CLOSED;
                            else if (is_open_after_fail && !is_closed_after_fail) {
                                 current_op_state = DoorOpState::IDLE_OPEN; timer_activity_start_ms = millis(); auto_close_is_scheduled = true;
                            } else current_op_state = DoorOpState::IDLE_AJAR;
                        }
                    }
        ...
        case DoorOpState::MOTOR_OPENING:
            {
                ...
                if (is_open_sensor_now_active) {
                    door_motor_stop(); Serial.println("DoorOpMgr_Task: Door fully open (sensor)."); //
                    current_op_state = DoorOpState::IDLE_OPEN; timer_activity_start_ms = millis();
                    auto_close_is_scheduled = true; Serial.println("DoorOpMgr_Task: Auto-close timer started for IDLE_OPEN state.");
                    ...
                    feedback_signal_error(); door_motor_stop();
                    current_op_state = DoorOpState::MOTOR_TIMEOUT; timer_activity_start_ms = millis();
                }
            }
            break;
        case DoorOpState::MOTOR_CLOSING:
            {
                ...
                if (is_closed_sensor_now_active) {
                    door_motor_stop(); Serial.println("DoorOpMgr_Task: Door physically closed (sensor).");
                    if (!last_valid_code_for_api_notify.isEmpty()) {
                        ApiResponse response = api_notify_door_close(last_valid_code_for_api_notify);
                        if(response.network_success && response.http_code == 201 && response.action_success) Serial.println("  -> API 'door_close' (finalize entry code) OK.");
                        else Serial.println("  -> API 'door_close' (finalize entry code) FAILED.");
                        last_valid_code_for_api_notify = "";
                    } else Serial.println("  -> Door closed. No entry code pending finalization.");
                    current_op_state = DoorOpState::IDLE_CLOSED; auto_close_is_scheduled = false;
                } else if (millis() - timer_activity_start_ms > MOTOR_OPERATION_MAX_DURATION_MS) {
            ...
            break;
        case DoorOpState::MOTOR_TIMEOUT: case DoorOpState::API_ERROR:
            {
                if (millis() - timer_activity_start_ms > ERROR_DISPLAY_HOLD_MS) {
                    display_update_input_code(""); display_clear_line(1); //
                    bool is_closed_after_error = door_sensor_is_door_fully_closed(); bool is_open_after_error = door_sensor_is_door_fully_opened();
                    if (is_closed_after_error && !is_open_after_error) current_op_state = DoorOpState::IDLE_CLOSED;
                    else if (is_open_after_error && !is_closed_after_error) {
                        current_op_state = DoorOpState::IDLE_OPEN; timer_activity_start_ms = millis(); auto_close_is_scheduled = true;
                    } else current_op_state = DoorOpState::IDLE_AJAR;
                }
            }
        ...
        case DoorOpState::LOCKED_OUT:
            {
                ...
                if (time_elapsed_in_lockout < LOCKOUT_DURATION_MS) {
                    ...
                    bool is_closed = door_sensor_is_door_fully_closed(); bool is_open = door_sensor_is_door_fully_opened();
                    if (is_closed && !is_open) current_op_state = DoorOpState::IDLE_CLOSED;
...
void door_op_request_entry(const String& ir_code) { //
    if (current_op_state == DoorOpState::LOCKED_OUT) {
        ...
        feedback_signal_error(); // Ou un feedback spécifique pour "système verrouillé"
        return;
    }
